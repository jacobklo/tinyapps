...
private const val TAG = "CalendarAnnouncementMainActivity"
    ...
    Column(modifier = modifier.padding(16.dp)) {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Column {
                // Increase minutes
                IconButton(onClick = {
                    val current = minutesStr.toIntOrNull() ?: 0
                    minutesStr = (current + 1).toString()
                }) {
                    Text("▲")
                }
                // Decrease minutes
                IconButton(onClick = {
                    val current = minutesStr.toIntOrNull() ?: 0
                    if (current > 0) minutesStr = (current - 1).toString()
                }) {
                    Text("▼")
                }
            }
