...
- [ ] **A rolling window must not allocate O(n squared).** NOTE: the original wording here said "cap the effective rolling size at the row count", and Task 13 proved that inert on values - `MemberSelectors.rolling` already clamps each window at the top of the table, so `rolling(999999)` and `rolling(5000)` over 5000 rows produce identical results and identical allocation. The cap was kept because it collapses every above-the-table spelling onto one `PartitionKey`, worth about 50 MB per extra such column at n=5000, but it does NOT fix the underlying cost. The real fix is representational: `PartitionResult.membersOfPartition` forces every window into its own `IntArray`, while `rolling` and `bucket` always produce a contiguous range. A sealed member type - `Range(start, endInclusive)` or `Indices(IntArray)` - drops rolling from O(n squared) to O(n). Not done; recorded as a known limitation. Measured cost today: one `rolling:999999` column over 5000 rows is 491 ms and +116 MB on the composition thread
